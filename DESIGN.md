...
At narrow widths, preserve the existing wrapping action row and canvas space. Anchors do not enlarge node layout, require hover, or introduce a separate mobile UI. Light/dark focus contrast, touch behavior, actual Tab order, and screen-reader announcement quality passed manual acceptance; details are recorded in `TODO.md`.
...
- **Inputs:** text inputs and select triggers are 36px, `md` radius, surface fill, 1px border, and visible focus ring. Component creation uses a compact neutral grid of native 40px icon-and-label buttons in common-first order: Service, Database, Cache, Queue, Client, Gateway, Storage, External service, then Generic. It wraps from roughly three columns on narrow screens through four or five at medium widths, expanding only while every label remains comfortably readable; it never forces a cramped row or horizontal overflow. Each action has an explicit accessible `Add [type] component` name, and its icon is decorative. Existing-component kind editing and connection-kind editing retain compact native text-only selects, with endpoint-based accessible names and conditional ID disambiguation only when visible endpoint names are ambiguous. These selects do not place icons inside options. Textareas/prompts have `lg` radius, generous padding, and a 72px minimum height.
...
- **Nodes:** nodes remain uniform rectangles, 160-200px wide, at least 48px tall, with `md` radius, a shared neutral surface, and a shared 1px border. The component name is 14px/600 primary text. A 12px muted kind row pairs a restrained 16px Lucide outline icon with a visible human-readable label. Kind icons are decorative wherever they supplement visible labels, including the creation picker; native text-only selects remain icon-free. Types do not receive unique colors, gradients, borders, or semantic shapes.
- **Edges:** use a 1.5-2px neutral directional stroke with small, clear, consistent arrowheads. Non-Generic connection semantics appear as compact 12px/600 text at or near the edge midpoint, using shared `surface`, `border`, and `text-secondary` tokens for a light/dark-safe neutral label treatment. Generic is visually unlabeled to limit clutter, but remains present in accessible descriptions. Semantic kinds never change edge color, iconography, animation, line pattern, stroke, or arrowhead. Every edge exposes an ARIA description containing current endpoint names and the human-readable semantic kind; visible labels supplement rather than replace that description. Selected edges use accent. Handles remain hidden or subtle until hover/connection mode.
