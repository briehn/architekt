...
The Project foundation, Domain graph foundation, Static Diagram Rendering, Interactive Node Movement, Component Creation and Deletion, Connection Creation, Connection Deletion, Persistence, Undo/Redo, and Component Renaming milestones are complete. The application has a framework-independent domain graph with components, directional connections, immutable graph operations, focused Vitest coverage, and a React Flow rendering path. ArchitectureEditor owns coordinated graph, layout, renderer metadata, history, and connection-feedback state. Users can create, delete, and rename components; drag nodes; create and delete directional connections; persist the current graph and layout locally; and undo or redo accepted edits. Custom nodes, layout logic, and AI integration have not been implemented.
...
Its compact control region creates ComponentId values with crypto.randomUUID() at the UI boundary, trims form input, and submits accepted component additions, deletions, and renames through pure editor-state operations. Inline rename drafts, validation feedback, and focus state remain local UI state; they never enter the graph, editor state, history, or persistence. `renameComponentInEditorState` delegates validation and mutation to the graph, preserves node-position and measurement references for a changed rename, and preserves the complete editor-state reference for an exact no-op. React Flow connection gestures are translated into ArchitectureConnection values with application-generated ConnectionId values, then admitted through the same editor-state and domain-graph path. Existing connections are listed from the canonical graph by directional endpoint names and removed through a pure editor-state operation; duplicate visible endpoint pairs receive technical-ID disambiguation only while ambiguous. React Flow nodes and connection rows therefore reflect current graph names without renderer-owned rename state. Connection rejection feedback is calculated together with the authoritative latest-state transition in one functional React update. React Flow selection and edge reconnection remain disabled.
...
The V1 persisted editor document is plain serializable data containing only a schema version, graph components, graph connections, and diagram node positions. Component names are already fields of persisted components, so renaming requires no schema change. It contains no Maps, class instances, React Flow nodes or edges, renderer measurements, viewport state, or transient UI state.
